...
const crudConfig = {
    autores: {
        ...
        columns:["ID","Nome","Situação"], payload:f=>({nome:f.nome,nacionalidade:f.nacionalidade,dataNascimento:f.dataNascimento,generoLiterario:f.generoLiterario}),
        row:o=>[`<strong>#${o.id}</strong>`,`<div class="record-title">${esc(o.nome)}</div>`,statusPill(o.ativa)],
        details:openAuthorDetails
    },
    categorias: {
...
function drawCrudRows(name,query=""){
    ...
    tbody.innerHTML=data.map(o=>{const active=isRecordActive(o),details=c.details?`<button class="button tiny ghost" type="button" data-details="${name}" data-id="${o.id}">Consultar</button>`:"";return `<tr>${c.row(o).map(v=>`<td>${v}</td>`).join("")}<td><div class="row-actions">${details}<button class="button tiny secondary" type="button" data-edit="${name}" data-id="${o.id}">Editar</button><button class="button tiny ${active?"danger":"primary"}" type="button" data-status="${name}" data-id="${o.id}" data-active="${active}">${active?"Desativar":"Reativar"}</button></div></td></tr>`}).join("");
    $$(`[data-details="${name}"]`,tbody).forEach(b=>b.onclick=()=>c.details(Number(b.dataset.id)));
...
function setupModal(){["#modalClose","#modalOk"].forEach(s=>$(s).onclick=closeModal);$("#modalBackdrop").onclick=e=>{if(e.target.id==="modalBackdrop")closeModal()};$("#readerDetailsClose").onclick=closeReaderDetails;$("#readerDetailsBackdrop").onclick=e=>{if(e.target.id==="readerDetailsBackdrop")closeReaderDetails()}}
...
function openReaderDetails(id){
    const o=(state.data.leitores||[]).find(x=>x.id===id);if(!o)return;
    $("#readerDetailsContent").innerHTML=`<div class="reader-details-heading"><div><span class="record-code">${esc(o.codigo||"Não gerado")}</span><h2 id="readerDetailsTitle">${esc(o.nome)}</h2><p>CPF ${esc(o.cpf)}</p></div>${statusPill(o.ativo)}</div><div class="details-grid"><div><small>ID interno</small><strong>#${o.id}</strong></div><div><small>Data de nascimento</small><strong>${dateBR(o.dataNascimento)}</strong></div><div><small>E-mail</small><strong class="detail-break">${esc(o.email||"Não informado")}</strong></div><div><small>Telefone</small><span class="phone-line"><strong>${esc(o.numeroTelefone||"Não informado")}</strong>${whatsappButton(o.numeroTelefone,o.nome)}</span></div><div class="detail-address"><small>Endereço completo</small><strong>${esc(formatAddress(o))}</strong></div></div>`;
    $("#readerDetailsBackdrop").classList.add("open");$("#readerDetailsBackdrop").setAttribute("aria-hidden","false")
}
function openAuthorDetails(id){
    const o=(state.data.autores||[]).find(x=>x.id===id);if(!o)return;
    $("#readerDetailsContent").innerHTML=`<div class="reader-details-heading"><div><span class="record-code">Autor #${o.id}</span><h2 id="readerDetailsTitle">${esc(o.nome)}</h2><p>Cadastro completo do autor</p></div>${statusPill(o.ativa)}</div><div class="details-grid"><div><small>ID interno</small><strong>#${o.id}</strong></div><div><small>Nacionalidade</small><strong>${esc(labels.nacionalidade[o.nacionalidade]||o.nacionalidade||"Não informada")}</strong></div><div><small>Data de nascimento</small><strong>${dateBR(o.dataNascimento)}</strong></div><div><small>Gênero literário</small><strong>${esc(labels.genero[o.generoLiterario]||o.generoLiterario||"Não informado")}</strong></div></div>`;
    $("#readerDetailsBackdrop").classList.add("open");$("#readerDetailsBackdrop").setAttribute("aria-hidden","false")
}
function openBookDetails(id){
    const o=(state.data.livros||[]).find(x=>x.id===id);if(!o)return;
    $("#readerDetailsContent").innerHTML=`<div class="reader-details-heading"><div><span class="record-code">Livro #${o.id}</span><h2 id="readerDetailsTitle">${esc(o.titulo)}</h2><p>${esc(o.subtitulo||"Sem subtítulo")}</p></div>${statusPill(o.ativo)}</div><div class="details-grid"><div><small>ID interno</small><strong>#${o.id}</strong></div><div><small>ISBN</small><strong>${esc(o.isbn||"Não informado")}</strong></div><div><small>Ano de publicação</small><strong>${esc(o.anoPublicacao||"Não informado")}</strong></div><div><small>Quantidade em estoque</small><strong>${o.quantidade} unidade${o.quantidade===1?"":"s"}</strong></div><div><small>Autor</small><strong>${esc(o.autor?.nome||"Não informado")}</strong></div><div><small>Categoria</small><strong>${esc(o.categoria?.nome||"Não informada")}</strong></div></div>`;
    $("#readerDetailsBackdrop").classList.add("open");$("#readerDetailsBackdrop").setAttribute("aria-hidden","false")
}
function closeReaderDetails(){$("#readerDetailsBackdrop").classList.remove("open");$("#readerDetailsBackdrop").setAttribute("aria-hidden","true")}
function formatAddress(o){return [o.rua,o.numeroCasa,o.complemento,o.bairro,o.cidade,o.estado,o.cep&&`CEP ${o.cep}`].filter(Boolean).join(", ")}
function whatsappUrl(phone){let digits=String(phone||"").replace(/\D/g,"");if(digits.length===10||digits.length===11)digits=`55${digits}`;return digits.length>=12?`https://wa.me/${digits}`:""}
function whatsappButton(phone,name){const url=whatsappUrl(phone);return url?`<a class="whatsapp-button" href="${url}" target="_blank" rel="noopener noreferrer" aria-label="Conversar com ${esc(name||"o leitor")} pelo WhatsApp" title="Abrir conversa no WhatsApp"><svg viewBox="0 0 16 16" aria-hidden="true"><path d="M13.601 2.326A7.85 7.85 0 0 0 7.994 0C3.627 0 .068 3.558.064 7.926c0 1.399.366 2.76 1.057 3.965L0 16l4.204-1.102a7.93 7.93 0 0 0 3.79.965h.004c4.368 0 7.926-3.558 7.93-7.93a7.9 7.9 0 0 0-2.327-5.607M7.998 14.527a6.58 6.58 0 0 1-3.356-.92l-.24-.144-2.494.654.666-2.433-.156-.251a6.56 6.56 0 0 1-1.007-3.505c0-3.637 2.96-6.597 6.591-6.597a6.56 6.56 0 0 1 4.66 1.931 6.56 6.56 0 0 1 1.928 4.66c-.004 3.639-2.961 6.605-6.592 6.605m3.615-4.934c-.197-.099-1.17-.578-1.353-.644-.182-.066-.315-.099-.445.099-.133.197-.514.644-.627.775-.116.132-.232.148-.43.05-.197-.1-.836-.308-1.592-.985-.59-.525-.985-1.175-1.101-1.372-.116-.198-.013-.305.087-.403.09-.088.197-.23.296-.345.1-.116.133-.198.198-.33.066-.132.033-.248-.016-.347-.05-.099-.445-1.076-.611-1.47-.16-.389-.323-.335-.445-.34-.116-.007-.248-.007-.38-.007a.73.73 0 0 0-.529.248c-.182.198-.69.677-.69 1.654s.71 1.916.81 2.049c.098.132 1.394 2.132 3.383 2.992.47.205.84.326 1.129.418.475.152.906.129 1.248.08.38-.058 1.171-.48 1.338-.943.164-.462.164-.858.114-.943-.049-.084-.182-.132-.38-.23"/></svg><span>WhatsApp</span></a>`:""}
