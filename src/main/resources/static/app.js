...
const state = { screen: "home", data: {}, editing: {}, permissionGranted: false };
const labels = {
...
const crudConfig = {
    autores: {
        ...
        columns:["Autor","Nacionalidade","Nascimento"], payload:f=>({nome:f.nome,nacionalidade:f.nacionalidade,dataNascimento:f.dataNascimento,generoLiterario:f.generoLiterario}),
        row:o=>[`<div class="record-title">${esc(o.nome)}</div><div class="record-subtitle">${esc(labels.genero[o.generoLiterario]||o.generoLiterario)}</div>`,esc(labels.nacionalidade[o.nacionalidade]||o.nacionalidade),dateBR(o.dataNascimento)]
    },
    categorias: {
        ...
        columns:["Categoria","Descrição","Situação"], payload:f=>({nome:f.nome,descricao:f.descricao}),
        row:o=>[`<div class="record-title">${esc(o.nome)}</div>`,esc(o.descricao||"Sem descrição"),statusPill(o.ativa)]
    },
    livros: {
        ...
        columns:["Livro","Autor / Categoria","Estoque"], payload:f=>({titulo:f.titulo,isbn:f.isbn,anoPublicacao:Number(f.anoPublicacao),quantidade:Number(f.quantidade),autorId:Number(f.autorId),categoriaId:Number(f.categoriaId)}),
        row:o=>[`<div class="record-title">${esc(o.titulo)}</div><div class="record-subtitle">ISBN ${esc(o.isbn)} • ${o.anoPublicacao}</div>`,`<div class="record-title">${esc(o.autor?.nome||"—")}</div><div class="record-subtitle">${esc(o.categoria?.nome||"Sem categoria")}</div>`,`<strong>${o.quantidade}</strong> unidade${o.quantidade===1?"":"s"}`],
    ...
    leitores: {
        ...
        columns:["Leitor","Contato","Situação"], payload:f=>({nome:f.nome,cpf:f.cpf,dataNascimento:f.dataNascimento,numeroTelefone:f.numeroTelefone,rua:f.rua,numeroCasa:f.numeroCasa,complemento:f.complemento||"",bairro:f.bairro,cidade:f.cidade,estado:f.estado.toUpperCase(),cep:f.cep}),
        row:o=>[`<div class="record-title">${esc(o.nome)}</div><div class="record-subtitle">CPF ${esc(o.cpf)}</div>`,`<div>${esc(o.numeroTelefone)}</div><div class="record-subtitle">${esc(o.cidade)} / ${esc(o.estado)}</div>`,statusPill(o.ativo)]
    }
};
...
async function navigate(name,push=true){
    ...
    if(name==="devolucao") await populateReaders("#returnReader");
...
function drawCrudRows(name,query=""){
    ...
    tbody.innerHTML=data.map(o=>`<tr>${c.row(o).map(v=>`<td>${v}</td>`).join("")}<td><div class="row-actions"><button class="button tiny secondary" type="button" data-edit="${name}" data-id="${o.id}">Editar</button><button class="button tiny danger" type="button" data-delete="${name}" data-id="${o.id}">Desativar</button></div></td></tr>`).join("");
    $$(`[data-edit="${name}"]`,tbody).forEach(b=>b.onclick=()=>editCrud(name,Number(b.dataset.id)));
    $$(`[data-delete="${name}"]`,tbody).forEach(b=>b.onclick=()=>deleteCrud(name,Number(b.dataset.id)));
}
...
function setupLoans(){
    $("#checkPermission").onclick=checkPermission;$("#loanTerm").onchange=calculateDueDate;$("#loanBook").onchange=updateLoanSteps;
...
async function prepareWithdrawal(){await Promise.all([populateReaders("#loanReader"),populateAvailableBooks()]);resetLoan()}
...
async function populateAvailableBooks(){try{const books=await api("/livros");state.data.livros=books;fillSelect("#loanBook",books.filter(x=>x.ativo!==false&&x.quantidade>0),"Selecione um livro",x=>`${x.titulo} • ${x.quantidade} disp.`)}catch(e){toast(e.message,"error")}}
async function checkPermission(){
    const id=Number($("#loanReader").value);if(!id){toast("Selecione um leitor.","error");return}
    try{const allowed=await api(`/emprestimos/permissao/${id}`);state.permissionGranted=allowed;const badge=$("#permissionBadge"),msg=$("#permissionMessage");badge.className=`permission ${allowed?"allowed":"denied"}`;badge.textContent=allowed?"Leitor permitido":"Empréstimo bloqueado";msg.textContent=allowed?"Cadastro regular. Selecione o livro e o prazo.":"O leitor está inativo ou possui empréstimo em atraso.";$("#loanBook").disabled=!allowed;$("#loanTerm").disabled=!allowed;$("#confirmLoan").disabled=!allowed;if(allowed){setLoanStep(2);calculateDueDate()}else{modal("Empréstimo não permitido",msg.textContent,"error")}}
    catch(e){modal("Não foi possível validar",e.message,"error")}
}
function resetLoan(){state.permissionGranted=false;const today=isoToday();$("#loanDate").value=today;$("#loanDueDate").value="";$("#loanBook").disabled=true;$("#loanTerm").disabled=true;$("#confirmLoan").disabled=true;$("#permissionBadge").className="permission neutral";$("#permissionBadge").textContent="Aguardando leitor";$("#permissionMessage").textContent="Selecione um leitor para verificar a situação.";setLoanStep(1)}
...
async function submitLoan(e){e.preventDefault();if(!state.permissionGranted)return;const payload={leitorId:Number($("#loanReader").value),livroId:Number($("#loanBook").value),prazoDias:Number($("#loanTerm").value)};try{const loan=await api("/emprestimos",{method:"POST",body:JSON.stringify(payload)});modal("Empréstimo confirmado",`${loan.livro?.titulo||"Livro"} deve ser devolvido até ${dateBR(loan.dataPrevistaDevolucao)}.`);e.target.reset();await populateAvailableBooks();loadDashboard()}catch(err){modal("Não foi possível emprestar",err.message,"error")}}
